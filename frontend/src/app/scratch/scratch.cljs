...
(defn card--world-test [get-val set-val]
  (su/card
   ...
   [[:h3 "TODO"]
    ...
    "Add a locomotive:"
    [VCV loc1 (locomotive/mk-entity "loc1" 1 1)]
    ...
    "Add and initialize another locomotive:"
    [VCV loc2 (locomotive/mk-entity "loc2" 3 1)]
    [VCV world4 (-> world3
                    (ecs/insert-object loc2)
                    (ecs/put-all-events [(ecs/mk-event (ecs/to loc2) ::ecs/init 0)])
                    (ecs/advance-until-time 0))]

    "Tell the locomotives to run:"
    [VCV world5 (-> world4
                    (ecs/put-all-events
    ...
    ;; TERAZ ZROBIC TAK:
    ;; Zaimpl. (advance-until) - z zadanym warunkiem, ktory bedzie sprawdzany po kazdym stepie
    ;; W ten sposob znajdziemy konkretny czas, kiedy stop powinien sie wydarzyc (tzn. kiedy
    ;; w collisions pojawia sie obie lokomotywy na tym samym tile).
    ;; Potem to juz tylko sprawa przejscia przez 1-2 stepy, gdzie mozna juz konkretnie
    ;; inspectowac i debugowac.
    ;; A ta sama metoda przyda sie na przyszlosc.
